...
if(type[page] == 1) {
	...
	abox_x,
	abox_y,
	abox_x-sprite_get_width(portrait_frame)*scale*1.75,
	abox_y-sprite_get_height(portrait_frame)*scale*tpl/8, false);
	draw_set_color(c_white);
	
	draw_sprite_ext(portrait_frame, 0,abox_x,abox_y, scale*tpl/8, -scale*1.75, 90, c_white, 1);
	...
	repeat(tpl){
		...
		var ypad = (ii+iy)*(string_height("W")/2);
		draw_set_font(fnt_dialogue_small);
		draw_text_ext_color(abox_x-sprite_get_width(portrait_frame)*scale*1.5, (abox_y-tpl*string_height("W"))+ypad, ctext, stringHeight, txtwidth, col, col, col, col, 1); 
